...
sys.path.insert(0, str(_REPO_ROOT / 'algorithms' / 'mome'))
sys.path.insert(0, str(_REPO_ROOT / 'algorithms' / 'map_elites'))
...
def _run_mo_cma_mae(args, oracle: PMOOracle, generator: MoleculeGenerator,
    ...
    _spec = importlib.util.spec_from_file_location(
        ...
        str(_REPO_ROOT / 'algorithms' / 'mo_cma_mae' / 'optimizer.py'),
    )
...
def _run_cma_mae(args, oracle: PMOOracle, generator: MoleculeGenerator,
    ...
    _spec = importlib.util.spec_from_file_location(
        'cma_mae_optimizer',
        str(_REPO_ROOT / 'algorithms' / 'cma_mae' / 'optimizer.py'),
    )
    op = importlib.util.module_from_spec(_spec)
    _spec.loader.exec_module(op)
...
def _run_smiles_lstm_hc(args, oracle: PMOOracle, generator: MoleculeGenerator,
    ...
    """
    import yaml
    sys.path.insert(0, str(_REPO_ROOT / 'algorithms' / 'pmo'))
