...
For multi-objective CMA-MAE see: algorithms/mo_cma_mae/
  ...
  python main.py --fitness-mode smartcadd --smartcadd-mode descriptors \\
    ...
    parser.add_argument('--encoding', type=str, default='smiles',
                        ...
                        help='Molecular string encoding (smiles, selfies, bigsmiles, or slices). '
                             'Note: slices is not supported for CMA-MAE.')
    ...
    # ----------------------------------------------------------------
    # Crystal (SLICES) path — set up eval interface and defaults first
    # ----------------------------------------------------------------
    crystal_eval = None
    if args.encoding == 'slices':
        ...
        # Default objective for crystals
        if args.objective is None:
            args.objective = 'formation_energy'
            if not args.maximize:
                args.minimize = True
        # Default measure bounds for crystals: n_sites (1–20), n_species (1–6)
        if args.measure_bounds is None:
    ...
    # Evaluation interface (molecular path — skipped for slices)
    if args.encoding == 'slices':
    ...
    _emb_tok = None          # tokenizer override (None = same as model)
    ...
    if args.encoding == 'bigsmiles' and _emb_model == 'DeepChem/ChemBERTa-77M-MTR':
        ...
        _emb_tok   = 'bert-base-uncased'   # MatText ships no tokenizer in its repo
    ...
    if args.encoding == 'slices':
        # For crystals the genotype IS the SLICES string — no SMILES decode needed
    ...
    def evaluate_solution(smiles: str):
        # ── Crystal (SLICES) path — bypasses RDKit ──────────────────────
        if args.encoding == 'slices':
            if smiles is None:
                return _error_props()
    ...
    def generate_fn():
        ...
        if not pop:
            return None
        if args.encoding == 'slices':
            return pop[0]   # SLICES string is the genotype directly
