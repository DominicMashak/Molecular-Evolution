...
def _morgan_embeddings(smiles_list: List[str], nbits: int = 2048) -> np.ndarray:
    ...
    Crystal (SLICES) genotypes are not valid SMILES — returns zero matrix."""
...
class CMAMaeOptimizer:
    ...
    def initialize(self) -> None:
        ...
        archive (required for CMA-MAE emitters to sample from). Tries up to
        ...
        if n_added == 0:
            print(f"WARNING: Initialisation complete but archive is still empty after "
                  f"{attempt} attempts. CMA-MAE emitters will use random mean vectors.")
        else:
