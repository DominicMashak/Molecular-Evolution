#!/usr/bin/env python3
"""
NSGA-III Molecular Optimisation using pymoo.

Evolves molecules in the transformer UMAP embedding space.  NSGA-III's
SBX crossover and polynomial mutation operate on real-valued embedding
vectors z.  Decoding z → molecule uses nearest-neighbour lookup in a
running pool followed by SMILES mutation — identical to CMA-MAE.
...
def main():
    parser = argparse.ArgumentParser(
        description="NSGA-III Molecular Optimisation (pymoo, latent-space genome)",
    ...
    parser.add_argument('--pop_size', type=int, default=None,
                        help='NSGA-III population size. '
                             ...
                             '(auto-computed from n_obj and --n-partitions).')
    parser.add_argument('--n_gen', type=int, default=300,
                        help='Number of NSGA-III generations (default: 300)')
    ...
    parser.add_argument('--output_dir', type=str, default="nsga3_results",
    ...
    parser.add_argument('--n-partitions', type=int, default=None,
                        help='Das-Dennis lattice partitions. Auto-scheduled from '
                             'n_obj when omitted: {2:12, 3:8, 4:6, 5:5, 6:4, 7:4, 8:3}')
    parser.add_argument('--sbx-eta', type=float, default=15.0,
                        help='SBX crossover distribution index (default: 15). '
                             'Higher = more exploitative offspring.')
    parser.add_argument('--sbx-prob', type=float, default=0.9,
                        help='SBX crossover probability per variable (default: 0.9)')
    ...
    for mode, val in optimize_parsed:
        if mode == 'target':
            parser.error(
                "'target:X' optimization is not yet supported in NSGA-III. "
    ...
    if args.pop_size is not None and args.pop_size < len(ref_dirs):
        print(
            ...
            "NSGA-III performance may degrade. Consider omitting --pop_size."
        )
