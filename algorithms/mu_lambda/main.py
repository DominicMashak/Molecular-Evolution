...
Examples:
  ...
  python main.py --recalculate mu_lambda_results/ --objective beta_mean
    ...
    if args.encoding == 'slices':
        ...
        if args.objective == 'beta_mean':
            args.objective = 'formation_energy'
        if not args.minimize and not args.maximize:
            args.minimize = True  # minimize formation_energy by default

    # Setup evaluation interface (skipped for slices — already set above)
    if args.encoding != 'slices':
