"""
CTAEA Optimizer — latent-space genome, pymoo selection.

The genome is a real-valued vector z in the UMAP-reduced transformer
embedding space (ChemBERTa / PolyBERT / MatText).  pymoo's CTAEA applies
...
CTAEA maintains two archives:
  - CA (Convergence Archive): tracks progress toward the Pareto front.
  - DA (Diversity Archive): promotes diversity across reference directions.
algorithm.opt returns the non-dominated front of the CA.
"""
...
class MolecularProblem(Problem):
    """
    Thin wrapper that supplies CTAEA with variable metadata.

    _evaluate is never called — evaluation is driven entirely by the
    CTAEAOptimizer ask/tell loop.  The bounds xl/xu come from the UMAP
...
class CTAEAOptimizer:
    """
    CTAEA optimizer over the transformer UMAP embedding space.

    pymoo handles:
      - Das-Dennis reference-point generation
      - Two-archive (CA + DA) selection with restricted mating
    ...
    def initialize(self) -> None:
        ...
        step() call evaluates the CTAEA initial population.
        """
        print(f"z_smiles pool pre-seeded with {len(self._z_smiles_pool)} "
              "molecules from embedder fitting.")
        print("CTAEA initial population will be evaluated in step 1.")

    # ── Step ─────────────────────────────────────────────────────────────────

    def step(self) -> Dict[str, Any]:
        """One CTAEA generation via pymoo ask/tell.
        ...
        4. Tell pymoo (triggers CA/DA survival + restricted mating update).
        5. Collect stats from current Pareto front (CA non-dominated front).

        algorithm.opt = CA non-dominated front (set by CADASurvival._updateCA).
        """
        ...
        # Stats from current Pareto front (CA non-dominated solutions)
    ...
    def run(
        ...
        """Main CTAEA optimisation loop."""
        self.initialize()
        print(f"\nRunning CTAEA for {n_generations} generations...")
        history = []
    ...
    def save_pareto_archive(self, generation: int) -> None:
        """Save current CTAEA Pareto front (CA non-dominated) to JSON in original units."""
        opt = self.algorithm.opt
