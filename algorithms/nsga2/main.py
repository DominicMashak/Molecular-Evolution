...
def validate_objectives(objectives, optimize_objectives):
    ...
    valid_objectives = {
        ...
        # Crystal (SLICES) objectives
        'formation_energy', 'bandgap', 'n_sites', 'n_species',
...
Examples:
  ...
  python main.py --fitness-mode smartcadd --smartcadd-mode docking \\
    ...
    # Set defaults for crystal encoding before validation
    if args.encoding == 'slices':
        if not args.objectives:
            args.objectives = ['formation_energy', 'n_species']
        if not args.optimize:
            args.optimize = ['minimize', 'maximize']
        args.calculator = args.calculator or 'xtb'  # placeholder (not used for crystals)

    # Validate required arguments for optimization mode
    if args.encoding != 'slices' and args.fitness_mode == 'qc' and not args.calculator:
    ...
    # Crystal evaluation interface (injected into optimizer when encoding='slices')
    crystal_eval_interface = None
    if args.encoding == 'slices':
        if getattr(args, 'crystal_tc', False):
            from molev_utils.crystal_tc_interface import CrystalTcInterface
            crystal_eval_interface = CrystalTcInterface(verbose=args.verbose,
                                                         tc_threshold=getattr(args, 'tc_threshold', 5.0))
        else:
            from molev_utils.crystal_qe_interface import CrystalQEInterface
            crystal_eval_interface = CrystalQEInterface(verbose=args.verbose,
    ...
    optimizer = NSGA2Optimizer(
        ...
        eval_interface=crystal_eval_interface,
    )
