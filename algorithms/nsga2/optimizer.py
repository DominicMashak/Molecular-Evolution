...
class NSGA2Optimizer:
    ...
    def __init__(self,
        ...
        if eval_interface is not None:
            # Use injected interface (e.g. CrystalEvaluationInterface for slices encoding)
            self.qc_interface = eval_interface
        elif self.fitness_mode == "smartcadd":
    ...
    def compute_objectives(self, smiles: str, atomic_numbers, positions) -> list:
        ...
        Supports: beta, beta_mean, natoms, energy, dipole, homo_lumo_gap,
                 ...
                 homo_lumo_gap_range_distance, and crystal objectives
                 (formation_energy, n_sites, n_species, volume, density, spacegroup, bandgap).
        """
        # Crystal path: slices encoding bypasses RDKit entirely
        if self.encoding == 'slices':
            result = self.qc_interface.calculate(smiles)
            obj_values = []
            for obj_name in self.objectives:
                obj_values.append(float(_safe_float(result.get(obj_name, 0.0), 0.0)))
            return obj_values
    ...
    def create_individual(self, smiles: str, generation: int) -> 'Individual':
        """Create an Individual from SMILES string (or SLICES string for crystal encoding)."""
        from individual import Individual
        from rdkit import Chem
        from rdkit.Chem import AllChem

        # Crystal path: SLICES encoding bypasses RDKit geometry embedding
        if self.encoding == 'slices':
            eval_result = self.qc_interface.calculate(smiles)
            objectives = self.compute_objectives(smiles, None, None)
            n_sites = int(_safe_float(eval_result.get('n_sites', 0), 0))
            return Individual(
                smiles=smiles,
                objectives=objectives,
                beta_surrogate=float(objectives[0]) if objectives else 0.0,
                natoms=n_sites,
                generation=generation,
                homo_lumo_gap=_safe_float(eval_result.get('bandgap', 0.0)),
                transition_dipole=0.0,
                oscillator_strength=0.0,
                gamma=_safe_float(eval_result.get('formation_energy', 0.0)),
                alpha_mean=0.0,
                qed=0.0, sa_score=10.0, docking_score=0.0,
                lipinski_violations=0, mol_weight=0.0, logp=0.0,
                tpsa=0.0, admet_pass=0.0,
            )
